...
import type { AdminUserSummary } from '@shared/admin-users';
...
@Component({
  ...
  imports: [RouterLink, DatePipe, PaginatorComponent, AsyncShellComponent],
...
export class AdminUsersListPage implements OnInit {
  ...
  protected readonly items = signal<AdminUserSummary[]>([]);
  ...
  protected load(page = this.page()): void {
    ...
    this.error.set('');
    this.adminUsersApi.list(this.tenantSlug, page, this.limit).subscribe({
      next: (res) => {
      ...
      error: (err) => {
        this.error.set(apiErrorMessage(err, 'Could not load admin users.'));
        this.loading.set(false);
      },
    });
  }
